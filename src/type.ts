 ...
 * @param target
 */
export function isPlainObject<T = Record<string, any>>(target: unknown): target is T {
  if (!isObject(target)) {
    return false
  }
 ...
 * @param target
 */
export function isGeneratorFunc(target: unknown): target is AsyncGeneratorFunction {
